...
namespace freelanceparse
{
   internal class Program
   {
      ...
      private static string token { get; set; } = "5404437422:AAHRbNEuoiRt3eR1Ylvbs7gvpQM7Uck33To";
      ...
      private static async void ClientMessage(object sender, MessageEventArgs e)
      {
         try {
            ...
            if (message.Text == "/start") {
            ...
            else if (message.Text == "🧾 Подписка" || message.Text == "/sub") {
               ConnectDB.LoadData(users);
               try {
            ...
            else if (message.Text == "🛒 Категории" || message.Text == "/category") {
               try {
                  ...
                  await client.SendTextMessageAsync(message.Chat.Id, "🛒 Категории", replyMarkup: keyborad);
               } catch { }
            }
               ...
               try {
                  ...
                  ConnectDB.Query("update User set categories = 'none' where user_id = " + message.Chat.Id.ToString() + ";");
      ...
      private static async void ParseKwork(string chatId)
      {
         ...
         for (int i = 0; i < sources.Length; i++) {
            ...
            try {
               ...
               html = wc.DownloadString(sources[i]);
               Thread.Sleep(3000);
            } catch { break; }
            for (int j = 0; j < 12; j++) {
               try {
